...
class Decoder(tf.keras.layers.Layer):
    ...
    def call(self, x, para_enc, enc_l, training, ranks, padding_mask_l):
        ...
        for i in range(self.num_layers):
            x, pw = self.dec_layers[i](x, para_enc, enc_l, training,
...
class MyModel(tf.keras.Model):
    ...
    def call(self, inp, training, ranks, tar_inp, tar_real=None, cal_pw=False):
        con_words, padding_mask_l, para_enc = self.encoder(inp, training, ranks)

        decoder_out, para_weights = self.decoder(tar_inp, para_enc,
        ...
        return vocab_dists, pw, para_enc
